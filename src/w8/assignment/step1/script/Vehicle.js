class Vehicle {
  ...
  cohesion(others) {
    ...
    if (cnt > 0) {
      ...
      steer.sub(this.pos); // 내 무리의 평균 위치에서 내 위치를 뺀다
      steer.setMag(this.speedMx); // 물체의 속도를 최고속도로 하여
      steer.sub(this.vel); // 구한 벡터값에서 내 속도벡터를 뺀다
  ...
  align(others) {
    ...
    if (cnt > 0) {
      ...
      steer.sub(this.vel); // 범위 내의 물체의 속도벡터에 내 속도벡터를 뺀다
      steer.limit(this.forceMx); // 힘의 최댓값은 forceMx
    }
    return steer; // 힘 더해주기
  }
    ...
    others.forEach((each) => {
      ...
      if (each !== this) {
        ...
        if (dist > 0 && this.rad + each.rad > dist) {
          // 두 원의 반지름이 두 원의 중심점 사이의 거리(물체 사이의 거리)보다 큰 경우 = 두 물체가 부딪힘
          const distNormal = dist / (this.rad + each.rad); // 물체 사이의 거리를 반지름의 합으로 나눈다
  ...
  display() {
    ...
    noStroke(); // 물체의 윤곽선 없음
    fill(this.color); // 물체의 색을 color로 채우기
